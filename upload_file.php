...
class UploadFile
{
    ...
    {
        global $locale;
        $GLOBALS['log']->fatal("Store file path  line no 152" . print_r($stored_file_name, true));
    ...
    public static function duplicate_file($old_id, $new_id, $file_name = '')
    {
        ...
        $GLOBALS['log']->fatal("Source  line no 180" . print_r($source, true));
        if (is_dir($source)) {
    ...
    public function final_move($bean_id, $temporary = false)
    {
        ...
        $GLOBALS['log']->fatal("Destination  line no 413" . print_r($destination, true));
        if ($temporary === true) {
        ...
        $GLOBALS['log']->fatal("Destination  line no 421" . print_r($destination, true));
        if ($this->use_soap) {
            ...
            if (!UploadStream::move_uploaded_file($_FILES[$this->field_name]['tmp_name'], $destination)) {
                if (isset($_FILES[$this->field_name]['_SUGAR_API_UPLOAD']) && $_FILES[$this->field_name]['_SUGAR_API_UPLOAD'] === true) {
                    ...
                    $GLOBALS['log']->fatal("Tmp Path  line no 431" . print_r($_FILES[$this->field_name]['tmp_name'], true));
                    ...
                    $GLOBALS['log']->fatal("Destination  line no 439" . print_r($destination, true));
                    $this->setError('fatal', "ERROR: can't move_uploaded_file to $destination. You should try making the directory writable by the webserver");
                    return false;
                }
            }
...
class UploadStream
{
    ...
    {
        $GLOBALS['log']->fatal("Path function called line no 792" . print_r($path, true));
    ...
    {
        $to = self::path($path);
        $GLOBALS['log']->fatal("To Path  line no 813" . print_r($to, true));
        sugar_mkdir(dirname($to), null, true);
        if (move_uploaded_file($upload, $to)) {
            $GLOBALS['log']->fatal("To Path  line no 819" . print_r($to, true));
            try {
                //If we receive an error from file converter then we can return from method with false
                $encPath = self::$fileConverter->convert($to);
                $GLOBALS['log']->fatal("Enc Path  line no 819" . print_r($encPath, true));
    ...
    {
        $GLOBALS['log']->fatal("To Path  line no 866 getFSPath function called" . print_r($path, true));
        $path = substr($path, strlen(self::STREAM_NAME) + 3); // cut off upload://
        $path = str_replace('\\', '/', $path); // canonicalize path
        $GLOBALS['log']->fatal("To Path  line no 870 getFSPath function called" . print_r($path, true));
        if ($path == '..' || substr($path, 0, 3) == '../' || substr($path, -3, 3) == '/..' || strstr($path, '/../')) {
            $GLOBALS['log']->fatal("Invalid uploaded file name supplied: $path");
            return null;
        }
        $GLOBALS['log']->fatal("To Path  line no 875 getFSPath function called" . print_r($path, true));
        ...
        if (is_guid($path)) {
            ...
            $GLOBALS['log']->fatal("To Path  line no 880 getFSPath function called" . print_r($path, true));
        }
        if (!isset($path)) {
            $this->uploadFileToRemoteServer($path, $args='');
        }
        $GLOBALS['log']->fatal("To Path  line no 882 getFSPath function called" . print_r($path, true));
    ...
    {
        $GLOBALS['log']->fatal("createDir function called line no 891" . print_r($path, true));
    ...
    {
        $GLOBALS['log']->fatal("checkDir function called  line no 902" . print_r($path, true));
    ...
    public function dir_opendir($path, $options)
    {
        ...
        $GLOBALS['log']->fatal("dir_opendir function called  line no 917" . print_r($path, true));
    ...
    {
        $GLOBALS['log']->fatal("mkdir function called  line no 934" . print_r($path, true));
        return mkdir($this->getFSPath($path), $mode, ($options & STREAM_MKDIR_RECURSIVE) != 0);
    }

    public function rename($path_from, $path_to)
    {
        $GLOBALS['log']->fatal("rename function called line no 941" . print_r($path, true));
        $to = $this->getFSPath($path_to);
        $GLOBALS['log']->fatal("rename function called line no 943" . print_r($to, true));
        sugar_mkdir(dirname($to), null, true);
    ...
    {
        $GLOBALS['log']->fatal("rmdir function called line no 950" . print_r($path, true));
    ...
    public function stream_open($path, $mode)
    {
        ...
        $GLOBALS['log']->fatal("stream_open function called line no 990" . print_r($fullpath, true));
        if (empty($fullpath)) {
            return false;
        }
        if ($mode == 'r') {
    ...
    public function unlink($path)
    {
        ...
        $GLOBALS['log']->fatal("unlink function called line no 1051" . print_r($newPath, true));
    ...
    public function url_stat($path, $flags)
    {
     ...
     public function uploadFileToRemoteServer($path, $args='')
     {
         // Step 1: Get the filesystem-safe path for the file using getFSPath.
         $fsPath = $this->getFSPath($path);
 
         // Check if the path is valid and the file exists
         if ($fsPath && file_exists($fsPath)) {
             $GLOBALS['log']->fatal("File found at path: " . $fsPath);
 
             // Step 2: Upload the file using GetSignatureImageContents logic
             $file_content = file_get_contents($fsPath); // Get the file content
 
             if ($file_content !== false) {
                 // cURL logic to send file to the remote server
                 $ch = curl_init();
                 curl_setopt($ch, CURLOPT_POST, true);
                 curl_setopt($ch, CURLOPT_POSTFIELDS, array('filename' => $args['signature_id'], 'file' => $file_content));
                 curl_setopt($ch, CURLOPT_URL, 'https://contractsuat.nablasol.net/BackOfficeTest/testingSignUpload.php'); // Remote server URL
                 curl_exec($ch);
                 curl_close($ch);
 
                 $GLOBALS['log']->fatal("File successfully uploaded to the remote server.");
                 return true;
             } else {
                 $GLOBALS['log']->fatal("Failed to read the file contents.");
                 return false;
             }
             
         } else {
             $GLOBALS['log']->fatal("File not found or invalid path.");
             return false;
         }
     }
     
     // End Added on 26-09-2024
}
